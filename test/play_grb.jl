...
for (t_ind, tii) in enumerate(prm.ts.time_keys[1:1])
    ...
    if dev in idx.J_pqe
        ...
        if dev in idx.pr_devs
            ...
            quasiGrad.@constraint(model, dev_q[tii] == prm.dev.q_0[dev]*u_sum + prm.dev.beta[dev]*dev_p[tii])
            =#
        else
...
#=
    ...
    quasiGrad.@constraint(model, u_su_dev[tii] + sum(u_sd_dev[tii_inst] for tii_inst in T_mndn; init=0.0) - 1.0 <= 0)

    # 2. Minimum uptime: zhat_mnup
    T_mnup = idx.Ts_mnup[dev][t_ind] # t_set = get_tminup(tii, dev, prm)
    quasiGrad.@constraint(model, u_sd_dev[tii] + sum(u_su_dev[tii_inst] for tii_inst in T_mnup; init=0.0) - 1.0 <= 0)
    ...
    #=
    quasiGrad.@constraint(model, dev_p[tii] - dev_p_previous
...
=#
    ...
    quasiGrad.@constraint(model,  dev_p_previous - dev_p[tii]
            - dt*(prm.dev.p_ramp_down_ub[dev]*u_on_dev[tii]
            +     prm.dev.p_shutdown_ramp_ub[dev]*(1.0-u_on_dev[tii])) <= 0)
   #=
    # 5. Regulation up: zhat_rgu
    quasiGrad.@constraint(model, p_rgu[tii] - prm.dev.p_reg_res_up_ub[dev]*u_on_dev[tii] <= 0)
    
    # 6. Regulation down: zhat_rgd
    quasiGrad.@constraint(model, p_rgd[tii] - prm.dev.p_reg_res_down_ub[dev]*u_on_dev[tii] <= 0)

    # 7. Synchronized reserve: zhat_scr
    quasiGrad.@constraint(model, p_rgu[tii] + p_scr[tii] - prm.dev.p_syn_res_ub[dev]*u_on_dev[tii] <= 0)
    
    # 8. Synchronized reserve: zhat_nsc
    quasiGrad.@constraint(model, p_nsc[tii] - prm.dev.p_nsyn_res_ub[dev]*(1.0 - u_on_dev[tii]) <= 0)
    
    # 9. Ramping reserve up (on): zhat_rruon
    quasiGrad.@constraint(model, p_rgu[tii] + p_scr[tii] + p_rru_on[tii] - prm.dev.p_ramp_res_up_online_ub[dev]*u_on_dev[tii] <= 0)

    # 10. Ramping reserve up (off): zhat_rruoff
    quasiGrad.@constraint(model, p_nsc[tii] + p_rru_off[tii] - prm.dev.p_ramp_res_up_offline_ub[dev]*(1.0-u_on_dev[tii]) <= 0)
    
    # 11. Ramping reserve down (on): zhat_rrdon
    quasiGrad.@constraint(model, p_rgd[tii] + p_rrd_on[tii] - prm.dev.p_ramp_res_down_online_ub[dev]*u_on_dev[tii] <= 0)

    # 12. Ramping reserve down (off): zhat_rrdoff
    quasiGrad.@constraint(model, p_rrd_off[tii] - prm.dev.p_ramp_res_down_offline_ub[dev]*(1-u_on_dev[tii]) <= 0)
    =#
    # Now, we must separate: producers vs consumers
    if dev in idx.pr_devs
        #=
        # 13p. Maximum reserve limits (producers): zhat_pmax
        quasiGrad.@constraint(model, p_on[tii] + p_rgu[tii] + p_scr[tii] + p_rru_on[tii] - prm.dev.p_ub[dev][t_ind]*u_on_dev[tii] <= 0)
    
        # 14p. Minimum reserve limits (producers): zhat_pmin
        quasiGrad.@constraint(model, prm.dev.p_lb[dev][t_ind]*u_on_dev[tii] + p_rrd_on[tii] + p_rgd[tii] - p_on[tii] <= 0)
        
        # 15p. Off reserve limits (producers): zhat_pmaxoff
        quasiGrad.@constraint(model, p_su[tii] + p_sd[tii] + p_nsc[tii] + p_rru_off[tii] - prm.dev.p_ub[dev][t_ind]*(1.0 - u_on_dev[tii]) <= 0)
        ...
        quasiGrad.@constraint(model, dev_q[tii] + q_qru[tii] - prm.dev.q_ub[dev][t_ind]*u_sum <= 0)

        # 17p. Minimum reactive power reserves (producers): zhat_qmin
        quasiGrad.@constraint(model, q_qrd[tii] + prm.dev.q_lb[dev][t_ind]*u_sum - dev_q[tii] <= 0)

        # 18p. Linked maximum reactive power reserves (producers): zhat_qmax_beta
        if dev in idx.J_pqmax
            quasiGrad.@constraint(model, dev_q[tii] + q_qru[tii] - prm.dev.q_0_ub[dev]*u_sum
            - prm.dev.beta_ub[dev]*dev_p[tii] <= 0)
        end 
        
        # 19p. Linked minimum reactive power reserves (producers): zhat_qmin_beta
        if dev in idx.J_pqmin
            quasiGrad.@constraint(model, prm.dev.q_0_lb[dev]*u_sum
    ...
    # consumers
        ...
        quasiGrad.@constraint(model, p_on[tii] + p_rgd[tii] + p_rrd_on[tii] - prm.dev.p_ub[dev][t_ind]*u_on_dev[tii] <= 0)

        # 14c. Minimum reserve limits (consumers): zhat_pmin
        #quasiGrad.@constraint(model, prm.dev.p_lb[dev][t_ind]*u_on_dev[tii] + p_rru_on[tii] + p_scr[tii] + p_rgu[tii] - p_on[tii] <= 0)
        
        # 15c. Off reserve limits (consumers): zhat_pmaxoff
        quasiGrad.@constraint(model, p_su[tii] + p_sd[tii] + p_rrd_off[tii] - prm.dev.p_ub[dev][t_ind]*(1.0 - u_on_dev[tii]) <= 0)
        ...
        quasiGrad.@constraint(model, dev_q[tii] + q_qrd[tii] - prm.dev.q_ub[dev][t_ind]*u_sum <= 0)

        # 17c. Minimum reactive power reserves (consumers): zhat_qmin
        quasiGrad.@constraint(model, q_qru[tii] + prm.dev.q_lb[dev][t_ind]*u_sum - dev_q[tii] <= 0)
        
        # 18c. Linked maximum reactive power reserves (consumers): zhat_qmax_beta
        if dev in idx.J_pqmax
            quasiGrad.@constraint(model, dev_q[tii] + q_qrd[tii] - prm.dev.q_0_ub[dev]*u_sum
            - prm.dev.beta_ub[dev]*dev_p[tii] <= 0)
        end 
...
for (w_ind, w_params) in enumerate(prm.dev.startups_ub[dev])
    ...
    quasiGrad.@constraint(model, sum(u_su_dev[tii] for tii in T_su_max; init=0.0) - w_params[3] <= 0.0)
end
=#
...
for (t_ind, tii) in enumerate(prm.ts.time_keys[1:1])
    if tii == :t1
        quasiGrad.@constraint(model, u_on_dev[tii] - prm.dev.init_on_status[dev] == u_su_dev[tii] - u_sd_dev[tii])
    else
        tii_m1 = prm.ts.time_keys[t_ind-1]
        quasiGrad.@constraint(model, u_on_dev[tii] - u_on_dev[tii_m1] == u_su_dev[tii] - u_sd_dev[tii])
    end
    # only one can be nonzero
    quasiGrad.@constraint(model, u_su_dev[tii] + u_sd_dev[tii] <= 1)
end
...
for (t_ind, tii) in enumerate(prm.ts.time_keys[1:1])
    ...
    if dev ∉ upd[:u_on_dev][tii]
        quasiGrad.@constraint(model, u_on_dev[tii] == stt[:u_on_dev][tii][dev])
    else
        # add it to the objective function
        tmp = quasiGrad.@variable(model)
        quasiGrad.@constraint(model, u_on_dev[tii]  - stt[:u_on_dev][tii][dev] <= tmp)
        quasiGrad.@constraint(model, stt[:u_on_dev][tii][dev] - u_on_dev[tii]  <= tmp)
        quasiGrad.add_to_expression!(obj, tmp, qG.binary_projection_weight)
    end

    if dev ∉ upd[:p_rrd_off][tii]
        quasiGrad.@constraint(model, p_rrd_off[tii] == stt[:p_rrd_off][tii][dev])
    else
        # add it to the objective function
        tmp = quasiGrad.@variable(model)
        quasiGrad.@constraint(model, p_rrd_off[tii] - stt[:p_rrd_off][tii][dev] <= tmp)
        quasiGrad.@constraint(model, stt[:p_rrd_off][tii][dev] - p_rrd_off[tii] <= tmp)
        quasiGrad.add_to_expression!(obj, tmp)
    end

    if dev ∉ upd[:p_nsc][tii]
        quasiGrad.@constraint(model, p_nsc[tii] == stt[:p_nsc][tii][dev])
    else
        # add it to the objective function
        tmp = quasiGrad.@variable(model)
        quasiGrad.@constraint(model, p_nsc[tii]  - stt[:p_nsc][tii][dev] <= tmp)
        quasiGrad.@constraint(model, stt[:p_nsc][tii][dev] - p_nsc[tii] <= tmp)
        quasiGrad.add_to_expression!(obj, tmp)
    end

    if dev ∉ upd[:p_rru_off][tii]
        quasiGrad.@constraint(model, p_rru_off[tii] == stt[:p_rru_off][tii][dev])
    else
        # add it to the objective function
        tmp = quasiGrad.@variable(model)
        quasiGrad.@constraint(model, p_rru_off[tii]  - stt[:p_rru_off][tii][dev] <= tmp)
        quasiGrad.@constraint(model, stt[:p_rru_off][tii][dev] - p_rru_off[tii]  <= tmp)
        quasiGrad.add_to_expression!(obj, tmp)
    end

    if dev ∉ upd[:q_qru][tii]
        quasiGrad.@constraint(model, q_qru[tii] == stt[:q_qru][tii][dev])
    else
        # add it to the objective function
        tmp = quasiGrad.@variable(model)
        quasiGrad.@constraint(model, q_qru[tii]  - stt[:q_qru][tii][dev] <= tmp)
        quasiGrad.@constraint(model, stt[:q_qru][tii][dev] - q_qru[tii]  <= tmp)
        quasiGrad.add_to_expression!(obj, tmp)
    end
    if dev ∉ upd[:q_qrd][tii]
        quasiGrad.@constraint(model, q_qrd[tii] == stt[:q_qrd][tii][dev])
    else
        # add it to the objective function
        tmp = quasiGrad.@variable(model)
        quasiGrad.@constraint(model, q_qrd[tii]  - stt[:q_qrd][tii][dev] <= tmp)
        quasiGrad.@constraint(model, stt[:q_qrd][tii][dev] - q_qrd[tii]  <= tmp)
        quasiGrad.add_to_expression!(obj, tmp)
    end
    ...
    if dev ∉ idx.J_pqe
        ...
        # add it to the objective function
        tmp = quasiGrad.@variable(model)
        quasiGrad.@constraint(model, dev_q[tii]  - stt[:dev_q][tii][dev] <= tmp)
        quasiGrad.@constraint(model, stt[:dev_q][tii][dev] - dev_q[tii]  <= tmp)
        quasiGrad.add_to_expression!(obj, tmp)
    end
    ...
    # p_on
    tmp = quasiGrad.@variable(model)
    quasiGrad.@constraint(model, p_on[tii]  - stt[:p_on][tii][dev] <= tmp)
    quasiGrad.@constraint(model, stt[:p_on][tii][dev] - p_on[tii]  <= tmp)
    quasiGrad.add_to_expression!(obj, tmp)
    
    # p_rgu 
    tmp = quasiGrad.@variable(model)
    quasiGrad.@constraint(model, p_rgu[tii]  - stt[:p_rgu][tii][dev] <= tmp)
    quasiGrad.@constraint(model, stt[:p_rgu][tii][dev] - p_rgu[tii]  <= tmp)
    quasiGrad.add_to_expression!(obj, tmp)
    
    # p_rgd
    tmp = quasiGrad.@variable(model)
    quasiGrad.@constraint(model, p_rgd[tii]  - stt[:p_rgd][tii][dev] <= tmp)
    quasiGrad.@constraint(model, stt[:p_rgd][tii][dev] - p_rgd[tii]  <= tmp)
    quasiGrad.add_to_expression!(obj, tmp)

    # p_scr
    tmp = quasiGrad.@variable(model)
    quasiGrad.@constraint(model, p_scr[tii]  - stt[:p_scr][tii][dev] <= tmp)
    quasiGrad.@constraint(model, stt[:p_scr][tii][dev] - p_scr[tii]  <= tmp)
    quasiGrad.add_to_expression!(obj, tmp)

    # p_rru_on
    tmp = quasiGrad.@variable(model)
    quasiGrad.@constraint(model, p_rru_on[tii]  - stt[:p_rru_on][tii][dev] <= tmp)
    quasiGrad.@constraint(model, stt[:p_rru_on][tii][dev] - p_rru_on[tii]  <= tmp)
    quasiGrad.add_to_expression!(obj, tmp)

    # p_rrd_on
    tmp = quasiGrad.@variable(model)
    quasiGrad.@constraint(model, p_rrd_on[tii]  - stt[:p_rrd_on][tii][dev] <= tmp)
    quasiGrad.@constraint(model, stt[:p_rrd_on][tii][dev] - p_rrd_on[tii]  <= tmp)
    quasiGrad.add_to_expression!(obj, tmp)
end

# set the objective
quasiGrad.@objective(model, Min, obj)
