# ac line flows
function acline_flows!(bit::Dict{Symbol, Dict{Symbol, BitVector}}, grd::Dict{Symbol, Dict{Symbol, Dict{Symbol, Vector{Float64}}}}, idx::quasiGrad.Idx, msc::Dict{Symbol, Dict{Symbol, Vector{Float64}}}, prm::quasiGrad.Param, qG::quasiGrad.QG, stt::Dict{Symbol, Dict{Symbol, Vector{Float64}}}, sys::quasiGrad.System)
    # line parameters
        ...
        if qG.eval_grad
            ...
            if qG.acflow_grad_is_soft_abs
                ...
                if sum(bit[:acline_sto_plus][tii]) > 0
                    msc[:acline_scale_fr][tii][bit[:acline_sto_plus][tii]]             .= msc[:acline_sto_plus][tii][bit[:acline_sto_plus][tii]]./sqrt.(msc[:acline_sto_plus][tii][bit[:acline_sto_plus][tii]].^2 .+ qG.acflow_grad_eps2);
                    grd[:zs_acline][:acline_pto][tii][bit[:acline_sto_plus][tii]] .= msc[:acline_scale_fr][tii][bit[:acline_sto_plus][tii]].*((dt*qG.acflow_grad_weight).*stt[:acline_pto][tii][bit[:acline_sto_plus][tii]]./msc[:acline_sto][tii][bit[:acline_sto_plus][tii]])
                    grd[:zs_acline][:acline_qto][tii][bit[:acline_sto_plus][tii]] .= msc[:acline_scale_fr][tii][bit[:acline_sto_plus][tii]].*((dt*qG.acflow_grad_weight).*stt[:acline_qto][tii][bit[:acline_sto_plus][tii]]./msc[:acline_sto][tii][bit[:acline_sto_plus][tii]])
                end
            else
                # gradients
...
# xfm line flows
# ac line flows
function xfm_flows!(bit::Dict{Symbol, Dict{Symbol, BitVector}}, grd::Dict{Symbol, Dict{Symbol, Dict{Symbol, Vector{Float64}}}}, idx::quasiGrad.Idx, msc::Dict{Symbol, Dict{Symbol, Vector{Float64}}}, prm::quasiGrad.Param, qG::quasiGrad.QG, stt::Dict{Symbol, Dict{Symbol, Vector{Float64}}}, sys::quasiGrad.System)
    g_sr = prm.xfm.g_sr
    b_sr = prm.xfm.b_sr
    b_ch = prm.xfm.b_ch
    g_fr = prm.xfm.g_fr
