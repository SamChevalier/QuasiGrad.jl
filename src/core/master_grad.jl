...
function master_grad_zs_acline_test!(tii::Int8, idx::quasiGrad.Idx, grd::quasiGrad.Grad, mgd::quasiGrad.Mgd, qG::quasiGrad.QG, sys::quasiGrad.System)
    ...
    # OG => mgpto   = mg_com.*pto_com
    # mgpto * everything below:
    vmfrpto = grd.zs_acline.acline_pto[tii].*grd.acline_pto.vmfr[tii]
    vmtopto = grd.zs_acline.acline_pto[tii].*grd.acline_pto.vmto[tii]
    vafrpto = grd.zs_acline.acline_pto[tii].*grd.acline_pto.vafr[tii]
    vatopto = grd.zs_acline.acline_pto[tii].*grd.acline_pto.vato[tii]

    # final qfr gradients
    # OG => mgqto   = mg_com.*qto_com
    # mgqto * everything below:
    vmfrqto = grd.zs_acline.acline_qto[tii].*grd.acline_qto.vmfr[tii]
    vmtoqto = grd.zs_acline.acline_qto[tii].*grd.acline_qto.vmto[tii]
    vafrqto = grd.zs_acline.acline_qto[tii].*grd.acline_qto.vafr[tii]
    vatoqto = grd.zs_acline.acline_qto[tii].*grd.acline_qto.vato[tii]
