...
function apply_q_injections!(idx::quasiGrad.Index, msc::quasiGrad.Msc, prm::quasiGrad.Param, qG::quasiGrad.QG, stt::quasiGrad.State, sys::quasiGrad.System)
    ...
    @floop ThreadedEx(basesize = qG.nT ÷ qG.num_threads) for tii in prm.ts.time_keys
        ...
        msc.dev_qlb[tii] .= stt.u_sum[tii].*prm.dev.q_lb_tmdv[tii]
        msc.dev_qub[tii] .= stt.u_sum[tii].*prm.dev.q_ub_tmdv[tii]
        ...
        for dev in idx.J_pqe
            msc.dev_qlb[tii][dev] = copy(stt.dev_q[tii][dev])
            msc.dev_qub[tii][dev] = copy(stt.dev_q[tii][dev])
        end
        ...
        for bus in 1:sys.nb
            ...
            qb_slack = 
                    # shunt        
                    sum(stt.sh_q[tii][sh] for sh in dcfr_qlb; init=0.0) +
                    # acline
            ...
            # get limits -- Q
            pr_Qlb   = sum(msc.dev_qlb[tii][pr] for pr  in idx.pr[bus]; init=0.0)
            cs_Qlb   = sum(msc.dev_qlb[tii][cs] for cs  in idx.cs[bus]; init=0.0)
            pr_Qub   = sum(msc.dev_qub[tii][pr] for pr  in idx.pr[bus]; init=0.0) 
            cs_Qub   = sum(msc.dev_qub[tii][cs] for cs  in idx.cs[bus]; init=0.0)
            ...
            if -qb_slack >= qub
                ...
                for cs in idx.cs[bus]
                    stt.dev_q[tii][cs] = copy(msc.dev_qub[tii][cs])
                end
                for pr in idx.pr[bus]
                    stt.dev_q[tii][pr] = copy(msc.dev_qlb[tii][pr])
                end
            ...
            elseif -qb_slack < qlb
                ...
                # min everything out
                for cs in idx.cs[bus]
                    stt.dev_q[tii][cs] = copy(msc.dev_qlb[tii][cs])
                end
                for pr in idx.pr[bus]
                    stt.dev_q[tii][pr] = copy(msc.dev_qub[tii][pr])
                end
                for dcl in idx.bus_is_dc_frs[bus]
                ...
                # apply
                for cs in idx.cs[bus]
                    stt.dev_q[tii][cs] = msc.dev_qlb[tii][cs] + scale*(msc.dev_qub[tii][cs] - msc.dev_qlb[tii][cs])
                end
                for pr in idx.pr[bus]
                    stt.dev_q[tii][pr] = msc.dev_qub[tii][pr] - scale*(msc.dev_qub[tii][pr] - msc.dev_qlb[tii][pr])
                end
                for dcl in idx.bus_is_dc_frs[bus]
...
function update_states_and_grads_for_solve_pf_lbfgs!(cgd::quasiGrad.ConstantGrad, grd::quasiGrad.Grad, idx::quasiGrad.Index, lbf::quasiGrad.LBFGS, mgd::quasiGrad.MasterGrad, msc::quasiGrad.Msc, prm::quasiGrad.Param, qG::quasiGrad.QG, stt::quasiGrad.State, sys::quasiGrad.System)
    ...
    quasiGrad.master_grad_solve_pf!(cgd, grd, idx, mgd, msc, prm, qG, stt, sys)
end
...
function solve_pf_lbfgs!(lbf::quasiGrad.LBFGS, mgd::quasiGrad.MasterGrad, prm::quasiGrad.Param, qG::quasiGrad.QG, stt::quasiGrad.State, upd::Dict{Symbol, Vector{Vector{Int64}}})
    ...
    if sum(lbf.idx) == 0
        ...
        for tii in prm.ts.time_keys
            # if this is the very first iteration, just take a gradient step
            lbf.state[:x_new][tii] .= lbf.state[:x_now][tii] .- 1e-16*lbf.state[:gradf_now][tii]
        end
        ...
        # => @floop ThreadedEx(basesize = qG.nT ÷ qG.num_threads) for tii in prm.ts.time_keys
            ...
            idx_km1                     = lbf.idx[1]
            lbf.diff[:s][tii][idx_km1] .= lbf.state[:x_now][tii]     .- lbf.state[:x_prev][tii]
            lbf.diff[:y][tii][idx_km1] .= lbf.state[:gradf_now][tii] .- lbf.state[:gradf_prev][tii]
            rho                         = quasiGrad.dot(lbf.diff[:s][tii][idx_km1], lbf.diff[:y][tii][idx_km1])
            if abs(rho) < 1e-6
            ...
            for ii in lbf.idx[lbf.idx .!= 0] # k-1, k-2, ..., k-m
                lbf.state[:alpha][tii][ii] = lbf.state[:rho][tii][ii]*quasiGrad.dot(lbf.diff[:s][tii][ii], lbf.state[:q][tii])
                lbf.state[:q][tii]       .-= lbf.state[:alpha][tii][ii].*lbf.diff[:y][tii][ii]
            end
            
            # set "r", which will be H*grad
            lbf.state[:r][tii] .= lbf.state[:q][tii].*(quasiGrad.dot(lbf.diff[:s][tii][idx_km1], lbf.diff[:y][tii][idx_km1])/quasiGrad.dot(lbf.diff[:y][tii][idx_km1], lbf.diff[:y][tii][idx_km1]))
            
            # compute H*grad
            for ii in reverse(lbf.idx[lbf.idx .!= 0]) # k-m, k-m+1, ..., k-1
                # skip beta -- defined implicitly below
                lbf.state[:r][tii] .+= lbf.diff[:s][tii][ii].*(lbf.state[:alpha][tii][ii] - lbf.state[:rho][tii][ii]*quasiGrad.dot(lbf.diff[:y][tii][ii], lbf.state[:r][tii]))
            end
...
function build_acpf_Jac_and_pq0(msc::quasiGrad.Msc, qG::quasiGrad.QG, stt::quasiGrad.State, sys::quasiGrad.System, tii::Int8, Ybus_real::quasiGrad.SparseArrays.SparseMatrixCSC{Float64, Int64}, Ybus_imag::quasiGrad.SparseArrays.SparseMatrixCSC{Float64, Int64})
...
function solve_power_flow!(cgd::quasiGrad.ConstantGrad, grd::quasiGrad.Grad, idx::quasiGrad.Index, lbf::quasiGrad.LBFGS, mgd::quasiGrad.MasterGrad, msc::quasiGrad.Msc, ntk::quasiGrad.Network, prm::quasiGrad.Param, qG::quasiGrad.QG, stt::quasiGrad.State, sys::quasiGrad.System, upd::Dict{Symbol, Vector{Vector{Int64}}})
