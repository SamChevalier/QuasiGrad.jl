...
function cleanup_constrained_pf_with_Gurobi_constant_dev_p!(    
    ...
    # freeze the smallest generators and loads -- release the largest 25% 
    ...
    Threads.@threads for tii in prm.ts.time_keys
        ...
        while run_pf == true
            ...
            # apply additional bounds: J_pqe (equality constraints)
            for dev in idx.J_pqe
                @constraint(model, dev_q_vars[dev] - prm.dev.beta[dev]*stt.dev_p[tii][dev] == prm.dev.q_0[dev]*stt.u_sum[tii][dev])
                # alternative: @constraint(model, dev_q_vars[idx.J_pqe] .== prm.dev.q_0[idx.J_pqe]*stt.u_sum[tii][idx.J_pqe] + prm.dev.beta[idx.J_pqe]*dev_p_vars[idx.J_pqe])
            end
            ...
            #   remember: idx.J_pqmax == idx.J_pqmin
            for dev in idx.J_pqmax
                @constraint(model, dev_q_vars[dev] <= prm.dev.q_0_ub[dev]*stt.u_sum[tii][dev] + prm.dev.beta_ub[dev]*stt.dev_p[tii][dev])
                @constraint(model, prm.dev.q_0_lb[dev]*stt.u_sum[tii][dev] + prm.dev.beta_lb[dev]*stt.dev_p[tii][dev] <= dev_q_vars[dev])
            end
    ...
    for tii in prm.ts.time_keys
        ...
        while run_pf == true
            ...
            if tii != prm.ts.time_keys[end]
                ...
                # fix a subset of devices
                @constraint(model, stt.dev_p[tii_p1][fixed_devs] .==  dev_p_vars_future[fixed_devs])
                ...
                # 1. ramp up
                @constraint(model, dev_p_vars_future[free_devs] .- dev_p_vars[free_devs] .- dt_future.*(prm.dev.p_ramp_up_ub[free_devs].*(stt.u_on_dev[tii_p1][free_devs] .- stt.u_su_dev[tii_p1][free_devs]) .+ prm.dev.p_startup_ramp_ub[free_devs].*(stt.u_su_dev[tii_p1][free_devs] .+ 1.0 .- stt.u_on_dev[tii_p1][free_devs])) .<= 0.0)
                # 2. ramp down
                @constraint(model, dev_p_vars[free_devs] .- dev_p_vars_future[free_devs] .- dt_future.*(prm.dev.p_ramp_down_ub[free_devs].*stt.u_on_dev[tii_p1][free_devs] .+ prm.dev.p_shutdown_ramp_ub[free_devs].*(1.0 .- stt.u_on_dev[tii_p1][free_devs])) .<= 0.0)
                # 3. pmax
                @constraint(model, p_on_future[free_devs] .<= prm.dev.p_ub_tmdv[tii_p1][free_devs].*stt.u_on_dev[tii_p1][free_devs])
                # 4. pmin
                @constraint(model, prm.dev.p_lb_tmdv[tii_p1][free_devs].*stt.u_on_dev[tii_p1][free_devs] .<= p_on_future[free_devs])
                # 5. qmax
                @constraint(model, dev_q_vars_future .<= prm.dev.q_ub_tmdv[tii_p1].*stt.u_sum[tii_p1])
                # 6. qmin
                @constraint(model, prm.dev.q_lb_tmdv[tii_p1].*stt.u_sum[tii_p1] .<= dev_q_vars_future)
                ...
                for dev in idx.J_pqmax
                    @constraint(model, dev_q_vars_future[dev] <= prm.dev.q_0_ub[dev]*stt.u_sum[tii_p1][dev] + prm.dev.beta_ub[dev]*dev_p_vars_future[dev])
                    @constraint(model, prm.dev.q_0_lb[dev]*stt.u_sum[tii_p1][dev] + prm.dev.beta_lb[dev]*dev_p_vars_future[dev] <= dev_q_vars_future[dev])
                end
                ...
                if tii != prm.ts.time_keys[end-1]
                    ...
                    # fix a subset of devices
                    @constraint(model, stt.dev_p[tii_p2][fixed_devs] .==  dev_p_vars_future_2[fixed_devs])
                    ...
                    # 1. ramp up
                    @constraint(model, dev_p_vars_future_2[free_devs] .- dev_p_vars_future[free_devs] .- dt_future_2.*(prm.dev.p_ramp_up_ub[free_devs].*(stt.u_on_dev[tii_p2][free_devs] .- stt.u_su_dev[tii_p2][free_devs]) .+ prm.dev.p_startup_ramp_ub[free_devs].*(stt.u_su_dev[tii_p2][free_devs] .+ 1.0 .- stt.u_on_dev[tii_p2][free_devs])) .<= 0.0)
                    # 2. ramp down
                    @constraint(model, dev_p_vars_future[free_devs] .- dev_p_vars_future_2[free_devs] .- dt_future_2.*(prm.dev.p_ramp_down_ub[free_devs].*stt.u_on_dev[tii_p2][free_devs] .+ prm.dev.p_shutdown_ramp_ub[free_devs].*(1.0 .- stt.u_on_dev[tii_p2][free_devs])) .<= 0.0)
                    # 3. pmax
                    @constraint(model, p_on_future_2[free_devs] .<= prm.dev.p_ub_tmdv[tii_p2][free_devs].*stt.u_on_dev[tii_p2][free_devs])
                    # 4. pmin
                    @constraint(model, prm.dev.p_lb_tmdv[tii_p2][free_devs].*stt.u_on_dev[tii_p2][free_devs] .<= p_on_future_2[free_devs])
                    # 5. qmax
                    @constraint(model, dev_q_vars_future_2 .<= prm.dev.q_ub_tmdv[tii_p2].*stt.u_sum[tii_p2])
                    # 6. qmin
                    @constraint(model, prm.dev.q_lb_tmdv[tii_p2].*stt.u_sum[tii_p2] .<= dev_q_vars_future_2)
                    ...
                    for dev in idx.J_pqmax
                        @constraint(model, dev_q_vars_future_2[dev] <= prm.dev.q_0_ub[dev]*stt.u_sum[tii_p2][dev] + prm.dev.beta_ub[dev]*dev_p_vars_future_2[dev])
                        @constraint(model, prm.dev.q_0_lb[dev]*stt.u_sum[tii_p2][dev] + prm.dev.beta_lb[dev]*dev_p_vars_future_2[dev] <= dev_q_vars_future_2[dev])
                    end
                    
                    # one more
                    if tii != prm.ts.time_keys[end-2]
                        # in D1, if tii < 17, then we do a double-look-ahead
                        #
                        # in this case, we want to make sure that whatever power values are chosen
                        # can also be used in the satisfaction of the next time period constraints
                        tii_p3      = prm.ts.time_keys[tii+3]
                        dt_future_3 = prm.ts.duration[tii_p3]
    
                        @variable(model, dev_p_vars_future_3[1:sys.ndev])
                        @variable(model, dev_q_vars_future_3[1:sys.ndev])
                        set_start_value.(dev_p_vars_future_3, stt.dev_p[tii_p3])
                        set_start_value.(dev_q_vars_future_3, stt.dev_q[tii_p3])
    
                        # fix a subset of devices
                        @constraint(model, stt.dev_p[tii_p3][fixed_devs] .==  dev_p_vars_future_3[fixed_devs])
    
                        # define p_on_future
                        p_on_future_3 = Vector{AffExpr}(undef, sys.ndev)
                        for dev in 1:sys.ndev
                            p_on_future_3[dev] = AffExpr(0.0)
                            add_to_expression!(p_on_future_3[dev], dev_p_vars_future_3[dev])
                            add_to_expression!(p_on_future_3[dev], -stt.p_su[tii_p3][dev] - stt.p_sd[tii_p3][dev])
                        end
    
                        # mapping:
                            # dt_future         =>    dt_future_2         =>    dt_future_3
                            # tii_p1            =>    tii_p2              =>    tii_p3
                            # p_on_future       =>    p_on_future_2       =>    p_on_future_3
                            # dev_p_vars_future =>    dev_p_vars_future_2 =>    dev_p_vars_future_3
                            # dev_q_vars_future =>    dev_q_vars_future_2 =>    dev_q_vars_future_3
                            # dev_p_vars        =>    dev_p_vars_future   =>    dev_p_vars_future_2
                            # dev_q_vars        =>    dev_q_vars_future   =>    dev_q_vars_future_2
    
                        # 1. ramp up
                        @constraint(model, dev_p_vars_future_3[free_devs] .- dev_q_vars_future_2[free_devs] .- dt_future_3.*(prm.dev.p_ramp_up_ub[free_devs].*(stt.u_on_dev[tii_p3][free_devs] .- stt.u_su_dev[tii_p3][free_devs]) .+ prm.dev.p_startup_ramp_ub[free_devs].*(stt.u_su_dev[tii_p3][free_devs] .+ 1.0 .- stt.u_on_dev[tii_p3][free_devs])) .<= 0.0)
                        # 2. ramp down
                        @constraint(model, dev_q_vars_future_2[free_devs] .- dev_p_vars_future_3[free_devs] .- dt_future_3.*(prm.dev.p_ramp_down_ub[free_devs].*stt.u_on_dev[tii_p3][free_devs] .+ prm.dev.p_shutdown_ramp_ub[free_devs].*(1.0 .- stt.u_on_dev[tii_p3][free_devs])) .<= 0.0)
                        # 3. pmax
                        @constraint(model, p_on_future_3[free_devs] .<= prm.dev.p_ub_tmdv[tii_p3][free_devs].*stt.u_on_dev[tii_p3][free_devs])
                        # 4. pmin
                        @constraint(model, prm.dev.p_lb_tmdv[tii_p3][free_devs].*stt.u_on_dev[tii_p3][free_devs] .<= p_on_future_3[free_devs])
                        # 5. qmax
                        @constraint(model, dev_q_vars_future_3 .<= prm.dev.q_ub_tmdv[tii_p3].*stt.u_sum[tii_p3])
                        # 6. qmin
                        @constraint(model, prm.dev.q_lb_tmdv[tii_p3].*stt.u_sum[tii_p3] .<= dev_q_vars_future_3)
                        # 7. apply additional bounds: J_pqe (equality constraints)
                        for dev in idx.J_pqe
                            @constraint(model, dev_q_vars_future_3[dev] - prm.dev.beta[dev]*dev_p_vars_future_3[dev] == prm.dev.q_0[dev]*stt.u_sum[tii_p3][dev])
                        end
                        # apply additional bounds: J_pqmin/max (inequality constraints)
                        for dev in idx.J_pqmax
                            @constraint(model, dev_q_vars_future_3[dev] <= prm.dev.q_0_ub[dev]*stt.u_sum[tii_p3][dev] + prm.dev.beta_ub[dev]*dev_p_vars_future_3[dev])
                            @constraint(model, prm.dev.q_0_lb[dev]*stt.u_sum[tii_p3][dev] + prm.dev.beta_lb[dev]*dev_p_vars_future_3[dev] <= dev_q_vars_future_3[dev])
                        end

                        # one more
                        if tii != prm.ts.time_keys[end-3]
                            # in D1, if tii < 17, then we do a double-look-ahead
                            #
                            # in this case, we want to make sure that whatever power values are chosen
                            # can also be used in the satisfaction of the next time period constraints
                            tii_p4      = prm.ts.time_keys[tii+4]
                            dt_future_4 = prm.ts.duration[tii_p4]
        
                            @variable(model, dev_p_vars_future_4[1:sys.ndev])
                            @variable(model, dev_q_vars_future_4[1:sys.ndev])
                            set_start_value.(dev_p_vars_future_4, stt.dev_p[tii_p4])
                            set_start_value.(dev_q_vars_future_4, stt.dev_q[tii_p4])
        
                            # fix a subset of devices
                            @constraint(model, stt.dev_p[tii_p4][fixed_devs] .==  dev_p_vars_future_4[fixed_devs])
        
                            # define p_on_future
                            p_on_future_3 = Vector{AffExpr}(undef, sys.ndev)
                            for dev in 1:sys.ndev
                                p_on_future_3[dev] = AffExpr(0.0)
                                add_to_expression!(p_on_future_3[dev], dev_p_vars_future_4[dev])
                                add_to_expression!(p_on_future_3[dev], -stt.p_su[tii_p4][dev] - stt.p_sd[tii_p4][dev])
                            end
        
                            # 1. ramp up
                            @constraint(model, dev_p_vars_future_4[free_devs] .- dev_p_vars_future_3[free_devs] .- dt_future_4.*(prm.dev.p_ramp_up_ub[free_devs].*(stt.u_on_dev[tii_p4][free_devs] .- stt.u_su_dev[tii_p4][free_devs]) .+ prm.dev.p_startup_ramp_ub[free_devs].*(stt.u_su_dev[tii_p4][free_devs] .+ 1.0 .- stt.u_on_dev[tii_p4][free_devs])) .<= 0.0)
                            # 2. ramp down
                            @constraint(model, dev_p_vars_future_3[free_devs] .- dev_p_vars_future_4[free_devs] .- dt_future_4.*(prm.dev.p_ramp_down_ub[free_devs].*stt.u_on_dev[tii_p4][free_devs] .+ prm.dev.p_shutdown_ramp_ub[free_devs].*(1.0 .- stt.u_on_dev[tii_p4][free_devs])) .<= 0.0)
                            # 3. pmax
                            @constraint(model, p_on_future_3[free_devs] .<= prm.dev.p_ub_tmdv[tii_p4][free_devs].*stt.u_on_dev[tii_p4][free_devs])
                            # 4. pmin
                            @constraint(model, prm.dev.p_lb_tmdv[tii_p4][free_devs].*stt.u_on_dev[tii_p4][free_devs] .<= p_on_future_3[free_devs])
                            # 5. qmax
                            @constraint(model, dev_q_vars_future_4 .<= prm.dev.q_ub_tmdv[tii_p4].*stt.u_sum[tii_p4])
                            # 6. qmin
                            @constraint(model, prm.dev.q_lb_tmdv[tii_p4].*stt.u_sum[tii_p4] .<= dev_q_vars_future_4)
                            # 7. apply additional bounds: J_pqe (equality constraints)
                            for dev in idx.J_pqe
                                @constraint(model, dev_q_vars_future_4[dev] - prm.dev.beta[dev]*dev_p_vars_future_4[dev] == prm.dev.q_0[dev]*stt.u_sum[tii_p4][dev])
                            end
                            # apply additional bounds: J_pqmin/max (inequality constraints)
                            for dev in idx.J_pqmax
                                @constraint(model, dev_q_vars_future_4[dev] <= prm.dev.q_0_ub[dev]*stt.u_sum[tii_p4][dev] + prm.dev.beta_ub[dev]*dev_p_vars_future_4[dev])
                                @constraint(model, prm.dev.q_0_lb[dev]*stt.u_sum[tii_p4][dev] + prm.dev.beta_lb[dev]*dev_p_vars_future_4[dev] <= dev_q_vars_future_4[dev])
                            end
                        end
                    end
                end
...
function cleanup_constrained_pf_with_Gurobi_parallelized_23kd1!(    
    ...
    Threads.@threads for tii in prm.ts.time_keys
        ...
        if soln_valid == true
            ...
            # the solution is NOT valid, so we should increase bounds and try again
            @warn "PENALIZED power flow cleanup failed at $(tii)! Solving with relaxed power balance."
        end
    end
end