...
tkeys = [:t1, :t2, :t3]
#vrb = Dict{Symbol, Vector{quasiGrad.JuMP.VariableRef}}(tkeys[ii] => quasiGrad.@variable(model,[1:sys.ndev],base_name = "u_on_dev_t$(ii)") for ii in 1:3)
vrb = Dict{Symbol, Dict{Symbol, quasiGrad.Vector{quasiGrad.JuMP.VariableRef}}}(:u_on_dev => Dict(tkeys[ii] => quasiGrad.@variable(model,[1:sys.ndev],base_name = "u_on_dev_t$(ii)") for ii in 1:3))
...
vars_new = Dict(dev => Dict{Symbol, quasiGrad.JuMP.VariableRef}(tkeys[ii] => quasiGrad.@variable(model, base_name = "u_on_dev_t$(ii)",  start=stt[:u_on_dev][tkeys[ii]][dev],  binary=true)  for ii in 1:(sys.nT)) for dev in 1:sys.ndev)

#vars = Dict{Symbol, quasiGrad.JuMP.VariableRef}(tkeys[ii] => quasiGrad.@variable(model, base_name = "u_on_dev_t$(ii)",  start=stt[:u_on_dev][tkeys[ii]][dev],  binary=true)  for ii in 1:(sys.nT))

# %%
u_on_dev1  = Dict(tkeys[ii] => Dict{Symbol, quasiGrad.JuMP.VariableRef}(tkeys[ii] => quasiGrad.@variable(model, base_name = "u_on_dev_t$(ii)_d$(dev)",  start=stt[:u_on_dev][tkeys[ii]][dev],  lower_bound = 0.0, upper_bound = 1.0) for dev in 1:sys.ndev) for ii in 1:(sys.nT))

# %%

u_on_dev4  = Dict{Symbol, Vector{quasiGrad.JuMP.VariableRef}}(tkeys[ii] => quasiGrad.@variable(model, base_name = "u_on_dev_t$(ii)", [dev = 1:sys.ndev], start=stt[:u_on_dev][tkeys[ii]][dev],  lower_bound = 0.0, upper_bound = 1.0) for ii in 1:(sys.nT))
